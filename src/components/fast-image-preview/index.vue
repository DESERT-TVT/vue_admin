			...
			<div class="preview-footer" @click.stop="preventDefault">
				<ul class="preview-footer-tools">
					<li v-if="imgList.length > 1" @click="handleSwitch('prev')">
						<fast-icon icon="fast-prev"></fast-icon>
					</li>

					<li @click="handleScale('reduce')">
						<fast-icon icon="fast-suoxiao"></fast-icon>
					</li>
					<li @click="handleScale('add')">
						<fast-icon icon="fast-fangda"></fast-icon>
					</li>
					<li @click="imgScale = 1">
						<fast-icon icon="fast-real-size"></fast-icon>
					</li>
					<li @click="handleRotate('left')">
						<fast-icon icon="fast-rotate-left"></fast-icon>
					</li>
					<li @click="handleRotate('right')">
						<fast-icon icon="fast-rotate-right"></fast-icon>
					</li>
					<li v-if="imgList.length > 1" @click="handleSwitch('next')">
						<fast-icon icon="fast-next"></fast-icon>
					</li>
				</ul>
			</div>
			<span class="close-icon" @click="show = false">
				<fast-icon icon="fast-close" color="#fff" style="font-size: 25px"></fast-icon>
			</span>
		</div>
	</div>
</template>

<script lang="ts" setup name="FastImagePreview">
