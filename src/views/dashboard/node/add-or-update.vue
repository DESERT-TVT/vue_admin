	...
	<el-dialog v-model="visible" :title="!dataForm.id ? $t('add') : $t('edit')" :close-on-click-modal="false" draggable width="30%">
		<el-form ref="dataFormRef" :model="dataForm" :rules="dataRules" label-width="auto" style="margin-top: 20px">
			...
			<el-form-item label="节点名称" prop="name">
				<el-input v-model="dataForm.name" placeholder="请输入节点名称" clearable></el-input>
			</el-form-item>
		</el-form>
		<template #footer>
...
import { addNodeSubmitApi, platformApi, PlatformList, updateNodeSubmitApi } from '@/api/dataStatistics'
...
const submitHandle = () => {
	dataFormRef.value?.validate((valid: boolean) => {
		...
		if (dataForm.id) {
			...
			updateNodeSubmitApi(dataForm).then(() => {
				ElMessage.success({
					message: '操作成功',
					duration: 1000,
					onClose: () => {
			...
			addNodeSubmitApi(dataForm).then(() => {
				ElMessage.success({
					message: '操作成功',
					duration: 1000,
					onClose: () => {
