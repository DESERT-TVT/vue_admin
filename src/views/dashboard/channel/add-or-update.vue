	...
	<el-dialog v-model="visible" :title="!dataForm.id ? $t('add') : $t('edit')" :close-on-click-modal="false" draggable width="30%">
		<el-form ref="dataFormRef" :model="dataForm" :rules="dataRules" label-width="auto" style="margin-top: 20px">
			...
			<el-form-item label="渠道名称" prop="name">
				<el-input v-model="dataForm.name" placeholder="请输入渠道名称" clearable></el-input>
			</el-form-item>
		</el-form>
		<template #footer>
...
import { addChannelSubmitApi, platformApi, PlatformList, updateChannelSubmitApi } from '@/api/dataStatistics'
...
const submitHandle = () => {
	dataFormRef.value?.validate((valid: boolean) => {
		...
		if (dataForm.id) {
			...
			updateChannelSubmitApi(dataForm).then(() => {
				ElMessage.success({
					message: '操作成功',
					duration: 1000,
					onClose: () => {
			...
			addChannelSubmitApi(dataForm).then(() => {
				ElMessage.success({
					message: '操作成功',
					duration: 1000,
					onClose: () => {
