<template>
	<el-dialog v-model="visible" :close-on-click-modal="false" title="发布朋友圈" width="50%" @close="closeDialogHandle">
		<el-form ref="dataFormRef" :model="dataForm" label-width="150px" @keyup.enter="submitHandle()">
			<!-- 文案输入 -->
			<el-form-item label="文案" prop="content">
				<el-input type="textarea" v-model="dataForm.content" placeholder="请输入文案" :autosize="{ minRows: 3, maxRows: 5 }" />
			</el-form-item>
	...
	if (coverImageFile.value && coverImageFile.value.length > 0) {
		for (let i = 0; i < coverImageFile.value.length; i++) {
			if (coverImageFile.value.at(i).raw) {
				...
				await useUploadOssApi(raw, 'cover/', null, suffix).then(r => dataForm.media.push(r))
