<template>
	<el-card class="layout-query">
		<el-form ref="queryRef" :inline="true" :model="state.queryForm" @keyup.enter="getDataList()">
			...
			<el-form-item prop="jobGroup">
				<fast-dict-select v-model="state.queryForm.jobGroup" dict-type="schedule_group" clearable placeholder="任务组名"></fast-dict-select>
			</el-form-item>
			<el-form-item prop="status">
				<fast-dict-select v-model="state.queryForm.status" dict-type="schedule_status" clearable placeholder="任务状态"></fast-dict-select>
			</el-form-item>
			<el-form-item>
	...
	<el-card>
		...
		<el-table v-loading="state.dataListLoading" :data="state.dataList" border class="layout-table" @selection-change="selectionChangeHandle">
			...
			<fast-dict-column prop="jobGroup" label="任务组名" dict-type="schedule_group"></fast-dict-column>
			...
			<fast-dict-column prop="status" label="状态" dict-type="schedule_status"></fast-dict-column>
			<el-table-column label="操作" fixed="right" header-align="center" align="center" width="200">
				<template #default="scope">
					<el-button v-auth="'schedule:update'" type="primary" link @click="addOrUpdateHandle(scope.row.id)">修改 </el-button>
					<el-button v-if="scope.row.status === 0" v-auth="'schedule:update'" type="primary" link @click="statusHandle(scope.row, 1)"
						>恢复
					</el-button>
					<el-button v-if="scope.row.status === 1" v-auth="'schedule:update'" type="primary" link @click="statusHandle(scope.row, 0)"
						>暂停
					</el-button>
...
import { useScheduleRunApi, useScheduleStatusApi } from '@/api/quartz/schedule'
