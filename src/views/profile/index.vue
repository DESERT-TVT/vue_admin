	...
	<el-row :gutter="20">
		<el-col :span="8">
			<el-card class="profile-card">
				...
				<div class="avatar">
					<fast-upload-image v-model:image-url="userStore.user.avatar" :drag="false" border-radius="50%" @success="handleSuccess">
						<template #empty>
							<el-icon>
								<Avatar />
							</el-icon>
				...
				<ul>
					<li>
						<fast-icon icon="icon-user" />
						用户名 <span>{{ userStore.user.username }}</span>
					</li>
					<li>
						<fast-icon icon="icon-idcard" />
						姓名 <span>{{ userStore.user.realName }}</span>
					</li>
					<li>
						<fast-icon icon="icon-phone" />
						手机号码 <span>{{ userStore.user.mobile }}</span>
					</li>
					<li>
						<fast-icon icon="icon-mail" />
					...
					<li>
						...
						创建日期 <span>{{ userStore.user.createTime }}</span>
					</li>
				</ul>
			</el-card>
		</el-col>
