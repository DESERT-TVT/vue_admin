    ...
    >
      ...
      <div class="top_tools" v-show="scaleView">
        <div class="user_avatar">
          ...
          <div class="care" v-if="!isCare" @click="careAbout(1)">
            {{ $t('call.apply') }}
          </div>
          <img :src="alreadyFollowingImg" v-else @click="careAbout(2)" />
        </div>
        <div class="tools">
...
import { isAttention } from '@/api/Attention';
...
import alreadyFollowingImg from '@/im-sdk/assets/alreadyFollowing.svg';
...
const careAbout = async (type: number) => {
  await isAttention({
    targetId: String(targetUser.value?.userId!),
    type: type,
  }).then(() => {
    isCare.value = type == 1 ? true : false;
  });
};
...
async function loadUserDetail() {
  ...
  if (!target) return;
    ...
    isCare.value = res?.data.data.isFollow ?? false;
    selfUser.value = await dataCenter.user.value;
}
